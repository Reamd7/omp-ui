...
/**
 * Settings section chrome + field/control primitives.
 *
 * Migrated from openchamber sections/shared/SettingsSection.tsx (originally a
 * 560-line mega-module). All 10 exports preserved in one file for parity.
 *
 * Decoupling changes:
 *   - openchamber typography classes (page-title / section-title / group-title /
 *     field-label / description / meta) → standard Tailwind utilities
 *     (text-2xl / text-lg / text-sm / text-xs + font-weight combos)
 *   - dropdownTriggerVariants() → static class string (no dropdown-trigger primitive dep)
 */
 ...
 * Original used dropdownTriggerVariants() — replaced with equivalent static classes.
 */
// eslint-disable-next-line react-refresh/only-export-components
export const SETTINGS_CUSTOM_TRIGGER_CLASS = cn(
  "inline-flex items-center justify-between gap-2 rounded-md border border-border bg-transparent px-3 py-1.5 text-sm text-foreground",
  "hover:bg-interactive-hover focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/50",
...
export const SETTINGS_NUMBER_UNIT_CLASS = "shrink-0 text-xs tabular-nums text-muted-foreground";
 ...
 * Prefer these over ad-hoc heading utilities.
 */
...
export const SettingsSection: React.FC<SettingsSectionProps> = ({
  ...
  const hasHeader =
    ...
    (headerAction !== null && headerAction !== undefined) ||
    (info !== null && info !== undefined);

  return (
    <section
      data-settings-item={settingsItem}
      className={cn("space-y-5", divider ? "border-t border-border/60 py-8" : "pb-8", className)}
    >
      {hasHeader ? (
...
export const SettingsTwoColumn: React.FC<SettingsTwoColumnProps> = ({ children, className }) => {
  return (
...
export const SettingsGroupTitle: React.FC<SettingsGroupTitleProps> = ({
  ...
  return <Tag className={cn(SETTINGS_GROUP_TITLE_CLASS, className)}>{children}</Tag>;
};
    ...
    <div data-settings-item={settingsItem} className={cn("space-y-2", className)}>
      <div className="space-y-0.5">
        <div className="flex items-center gap-1.5">
          ...
          {info !== null && info !== undefined ? <SettingsInfoHint>{info}</SettingsInfoHint> : null}
        </div>
        {descriptionPlacement === "before" ? descriptionNode : null}
      </div>
      <div className={cn("flex min-w-0 max-w-[24rem] items-center gap-2", controlClassName)}>
        {children}
      </div>
...
export const SettingsInset: React.FC<SettingsInsetProps> = ({
  ...
  return (
    <div data-settings-item={settingsItem} className={cn("pt-4", className)}>
      {children}
    </div>
  );
};
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 flex-col">
        ...
        {hasDescription ? <span className={SETTINGS_HELPER_CLASS}>{description}</span> : null}
      </div>
    </div>
  );
};
  ...
  return (
    <div
      className={cn(
        ...
        description !== null && description !== undefined ? "items-start" : "items-center",
        disabled && "cursor-not-allowed opacity-60",
        className,
      )}
      role="button"
    ...
    >
      <Radio
        ...
        disabled={disabled}
        ariaLabel={ariaLabel}
        className={description !== null && description !== undefined ? "mt-0.5" : undefined}
      />
      <div className="flex min-w-0 flex-col">
        <span
          className={cn("text-sm font-normal", selected ? "text-foreground" : "text-foreground/50")}
        >
          {label}
        </span>
        {description !== null && description !== undefined ? (
...
export const SettingsRadioGroup: React.FC<SettingsRadioGroupProps> = ({
  ...
  return (
    <div
      role="radiogroup"
      aria-label={ariaLabel}
      className={cn(SETTINGS_OPTION_STACK_CLASS, className)}
    >
      {children}
    </div>
  );
